...
use std::error::Error;
use regex::Regex;
use crate::models::{Child, Day, TimeRange};
...
pub fn parse_planning(path: &str, year: i32) -> Result<Vec<Day>, Box<dyn Error>> {
    // Lire le fichier PDF
    ...
    for raw_line in parsed_text.lines() {
        ...
        if let Some(caps) = day_re.captures(line) {
            ...
            if let (Some(d_num), Some(month), Some(ref abbr)) = (current_day_num, current_month, &current_day_abbr) {
                ...
                if let Some(day) = process_day(&date, &jour, &current_lines)? {
                    days.push(day);
                }
            }
            // Nouveau jour
...
fn process_day(date: &str, jour: &str, lines: &[String]) -> Result<Option<Day>, Box<dyn Error>> {
    ...
    let am_shifts = compute_assistant_shifts(&enfants, 4);
